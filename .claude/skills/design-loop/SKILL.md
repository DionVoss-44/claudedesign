---
name: design-loop
description: Adversarial quality loop for creative work — a builder plus three fresh-context critics (brief, system, craft) judge rendered output against a real-world reference bar until all three pass. Use ONLY when explicitly invoked - the user types /design-loop, says "run the design loop" / "critic loop" / "gauntlet loop", asks to "loop this against [a reference]", or is following the Jack Roberts video workflow that prescribes it (gold-standard comparison of a build against a reference site). Do NOT trigger for ordinary design, build, or styling requests that lack an explicit ask for the loop.
---
